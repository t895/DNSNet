...
pub struct DnsCache {
    cache: RwLock<LruCache<DnsCacheKey, DnsCacheEntry>>,
}

impl DnsCache {
    pub fn new() -> Self {
        Self {
            cache: RwLock::new(LruCache::new(NonZeroUsize::new(1_000).unwrap())),
        }
    }
    ...
    pub fn get_packet(&self, packet: Packet<'_>) -> Option<Vec<u8>> {
        let mut cache = match self.cache.write() {
            Ok(cache) => cache,
            Err(error) => {
                error!("get_packet: Failed to get cache lock! - {:?}", error);
                return None;
            }
        };
        ...
        let entry = if let Some(entry) = (*cache).get(&ipv4_key) {
            entry
        } else if let Some(entry) = (*cache).get(&ipv6_key) {
            entry
        } else {
            return None;
        };
        ...
        if entry.expiry_time() > now {
            match entry.ip_record {
                IpAddr::V4(_) => (*cache).pop(&ipv4_key),
                IpAddr::V6(_) => (*cache).pop(&ipv6_key),
            };
            return None;
        }
    ...
    pub fn put_packet(&self, packet: &[u8]) {
        ...
        let mut cache: std::sync::RwLockWriteGuard<'_, LruCache<DnsCacheKey, DnsCacheEntry>> =
            match self.cache.write() {
                Ok(cache) => cache,
                Err(error) => {
                    error!("put_packet: Failed to get lock for cache! - {:?}", error);
                    return;
                }
            };

        Self::check_records(&mut cache, &packet.answers);
        Self::check_records(&mut cache, &packet.name_servers);
        Self::check_records(&mut cache, &packet.additional_records);
    }
    ...
    pub fn put_answer(&self, host_name: &str, address: &[u8]) {
        let mut cache: std::sync::RwLockWriteGuard<'_, LruCache<DnsCacheKey, DnsCacheEntry>> =
            match self.cache.write() {
                Ok(cache) => cache,
                Err(error) => {
                    error!("put_answer: Failed to get lock for cache! - {:?}", error);
                    return;
                }
            };
        ...
        match self.cache.write() {
            Ok(cache) => {
                let mut new_cache = SerializableDnsCache {
                    cache: Vec::with_capacity((&cache).len()),
                };
            ...
            Err(error) => {
                error!("to_disk_cache: Failed to serialize cache! - {:?}", error);
...
impl SerializableDnsCache {
    ...
    fn to_live_cache(self) -> DnsCache {
        let new_cache = DnsCache::new();
        if let Ok(mut cache) = new_cache.cache.write() {
